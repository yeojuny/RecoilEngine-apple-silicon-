...
inline size_t StablePosAllocator<T>::Allocate(size_t numElems)
{
	...
	if (positionToSize.empty()) {
		...
		myLog("StablePosAllocator<T>::Allocate(%u) = %u [thread_id = %u]", uint32_t(numElems), uint32_t(returnPos), static_cast<uint32_t>(reinterpret_cast<uintptr_t>(Threading::GetCurrentThreadId())));
		return returnPos;
	}
