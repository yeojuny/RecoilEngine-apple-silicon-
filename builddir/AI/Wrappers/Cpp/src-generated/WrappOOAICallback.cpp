...
struct SSkirmishAICallback;
	...
	int springai::WrappOOAICallback::GetNumTeams() {

		int internal_ret_int;

		internal_ret_int = bridged_getNumTeams(this->GetSkirmishAIId());
		return internal_ret_int;
	}

	int springai::WrappOOAICallback::GetNumSkirmishAIs() {

		int internal_ret_int;

		internal_ret_int = bridged_getNumSkirmishAIs(this->GetSkirmishAIId());
		return internal_ret_int;
	}

	int springai::WrappOOAICallback::GetMaxSkirmishAIs() {

		int internal_ret_int;

		internal_ret_int = bridged_getMaxSkirmishAIs(this->GetSkirmishAIId());
		return internal_ret_int;
	}
	...
	std::vector<springai::Unit*> springai::WrappOOAICallback::GetEnemyUnitsIn(const springai::AIFloat3& pos, float radius, bool spherical) {
		...
		unitIds_sizeMax = INT_MAX;
		unitIds = NULL;
		unitIds_size = bridged_getEnemyUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_sizeMax = unitIds_size;
		unitIds_raw_size = unitIds_size;
		unitIds = new int[unitIds_raw_size];

		internal_ret_int = bridged_getEnemyUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_list.reserve(unitIds_size);
		for (int i=0; i < unitIds_sizeMax; ++i) {
			unitIds_list.push_back(springai::WrappUnit::GetInstance(skirmishAIId, unitIds[i]));
		}
	...
	std::vector<springai::Unit*> springai::WrappOOAICallback::GetFriendlyUnitsIn(const springai::AIFloat3& pos, float radius, bool spherical) {
		...
		unitIds_sizeMax = INT_MAX;
		unitIds = NULL;
		unitIds_size = bridged_getFriendlyUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_sizeMax = unitIds_size;
		unitIds_raw_size = unitIds_size;
		unitIds = new int[unitIds_raw_size];

		internal_ret_int = bridged_getFriendlyUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_list.reserve(unitIds_size);
		for (int i=0; i < unitIds_sizeMax; ++i) {
			unitIds_list.push_back(springai::WrappUnit::GetInstance(skirmishAIId, unitIds[i]));
		}
	...
	std::vector<springai::Unit*> springai::WrappOOAICallback::GetNeutralUnitsIn(const springai::AIFloat3& pos, float radius, bool spherical) {
		...
		unitIds_sizeMax = INT_MAX;
		unitIds = NULL;
		unitIds_size = bridged_getNeutralUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_sizeMax = unitIds_size;
		unitIds_raw_size = unitIds_size;
		unitIds = new int[unitIds_raw_size];

		internal_ret_int = bridged_getNeutralUnitsIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, unitIds, unitIds_sizeMax);
		unitIds_list.reserve(unitIds_size);
		for (int i=0; i < unitIds_sizeMax; ++i) {
			unitIds_list.push_back(springai::WrappUnit::GetInstance(skirmishAIId, unitIds[i]));
		}
	...
	std::vector<springai::Team*> springai::WrappOOAICallback::GetAlliedTeams() {
		...
		teamIds_sizeMax = INT_MAX;
		teamIds = NULL;
		teamIds_size = bridged_getAlliedTeams(this->GetSkirmishAIId(), teamIds, teamIds_sizeMax);
		teamIds_sizeMax = teamIds_size;
		teamIds_raw_size = teamIds_size;
		teamIds = new int[teamIds_raw_size];

		internal_ret_int = bridged_getAlliedTeams(this->GetSkirmishAIId(), teamIds, teamIds_sizeMax);
		teamIds_list.reserve(teamIds_size);
		for (int i=0; i < teamIds_sizeMax; ++i) {
			teamIds_list.push_back(springai::WrappTeam::GetInstance(skirmishAIId, teamIds[i]));
		}
	...
	std::vector<springai::Feature*> springai::WrappOOAICallback::GetFeaturesIn(const springai::AIFloat3& pos, float radius, bool spherical) {
		...
		featureIds_sizeMax = INT_MAX;
		featureIds = NULL;
		featureIds_size = bridged_getFeaturesIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, featureIds, featureIds_sizeMax);
		featureIds_sizeMax = featureIds_size;
		featureIds_raw_size = featureIds_size;
		featureIds = new int[featureIds_raw_size];

		internal_ret_int = bridged_getFeaturesIn(this->GetSkirmishAIId(), pos_posF3, radius, spherical, featureIds, featureIds_sizeMax);
		featureIds_list.reserve(featureIds_size);
		for (int i=0; i < featureIds_sizeMax; ++i) {
			featureIds_list.push_back(springai::WrappFeature::GetInstance(skirmishAIId, featureIds[i]));
		}
